...
class DataStorePersistenceTest {
    ...
    @Test
    fun `weather cache read returns null on empty store`(
        @TempDir dir: File,
    ...
    @Test
    fun `weather cache round-trips a snapshot`(
        @TempDir dir: File,
    ) = runTest(UnconfinedTestDispatcher()) {
        val cache = DataStoreWeatherCache(backgroundScope.store(dir, "weather.preferences_pb"))
        val snap =
    ...
    @Test
    fun `settings update reflects every change`(
        @TempDir dir: File,
    ) = runTest(UnconfinedTestDispatcher()) {
        val settings: SettingsStore =
            DataStoreSettingsStore(backgroundScope.store(dir, "settings.preferences_pb"))
    ...
    @Test
    fun `icon colour round-trips and null removes the key`(
        @TempDir dir: File,
    ) = runTest(UnconfinedTestDispatcher()) {
        val settings: SettingsStore =
            DataStoreSettingsStore(backgroundScope.store(dir, "settings.preferences_pb"))
