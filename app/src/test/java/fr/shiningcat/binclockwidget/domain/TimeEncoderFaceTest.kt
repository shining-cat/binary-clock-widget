...
import fr.shiningcat.binclockwidget.domain.model.*
...
class TimeEncoderFaceTest {
  ...
  @Test fun `hours row has alarm glyph at col32 then 13 in 5 bits`() {
    ...
    assertEquals(listOf(false, true, true, false, true),
      row.cells.drop(1).map { (it as Cell.Bit).lit })   // 16,8,4,2,1 -> 8+4+1
  }
